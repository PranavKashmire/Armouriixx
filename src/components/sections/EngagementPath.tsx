  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 60% 50% at 50% 0%, rgba(212, 175, 55, 0.06) 0%, transparent 65%)",
        }}
      />
          ...
          {siteConfig.engagementSteps.map((step, i) => (
            <BlurFadeIn key={step.step} delay={i * 60}>
              <div className="relative flex gap-4 pb-6 last:pb-0">
                ...
                <div
                  className="flex-1 min-w-0 p-4 rounded-sm border border-[var(--border-subtle)] bg-[var(--ink-3)]/60"
                >
                  <span
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {siteConfig.engagementSteps.map((step, i) => (
            <BlurFadeIn key={step.step} delay={i * 80}>
              <div
                className="group relative h-full p-5 sm:p-6 rounded-sm border border-[var(--border-subtle)] bg-[var(--ink-3)]/60 hover:border-[var(--border-accent)] transition-all duration-500 hover:-translate-y-1"
              >
                <div className="flex items-center justify-between mb-4 gap-2">
                  <span
                    className="font-[var(--font-display)] text-2xl text-[var(--gold)]/25 group-hover:text-[var(--gold)]/50 transition-colors"
                  >
                    {step.step}
                  </span>
                  <span
