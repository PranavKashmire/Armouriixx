...
const DepthCarousel = ({
  ...
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (ov)
        ov.style.opacity = clamp(back * (cfg.falloff as number) * 1.25, 0, 0.86).toFixed(3);
    }
  }, []);
  ...
  // External index control — syncs carousel to TrueFocus word index
  useEffect(() => {
    if (forceIndex === undefined || forceIndex === null) return;
    setFocus(forceIndex, true);
