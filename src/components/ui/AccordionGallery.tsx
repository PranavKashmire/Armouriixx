...
interface AccordionGalleryProps {
  items?: AccordionItem[];
  defaultIndex?: number;
  /** Controlled active index — when provided, drives the accordion externally */
  activeIndex?: number;
  /** Callback fired when the user changes active panel */
...
export default function AccordionGallery({
  ...
  const [active, setActiveInternal] = useState(Math.min(Math.max(activeIndex ?? defaultIndex, 0), count - 1));

  // Sync when controlled activeIndex changes from outside
  useEffect(() => {
    if (activeIndex !== undefined) {
      setActiveInternal(Math.min(Math.max(activeIndex, 0), count - 1));
    }
  }, [activeIndex, count]);
