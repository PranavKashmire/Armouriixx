...
// Module-level guard: track if a renderer is being torn down to prevent
// the browser blocking new WebGL context creation during Fast Refresh
let _activeCleanupTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uMouse: { value: currentMouseRef.current },
    ...
    return () => {
      ...
      // Delay DOM/GL cleanup slightly so the browser releases the context
      // before React tries to re-create it on Fast Refresh
      const capturedRenderer = rendererRef.current;
      const capturedCanvas = capturedRenderer?.domElement;
      const capturedMaterial = materialRef.current;
      const capturedGeometry = geometryRef.current;
      ...
      if (_activeCleanupTimer) clearTimeout(_activeCleanupTimer);
      _activeCleanupTimer = setTimeout(() => {
        if (capturedRenderer) {
          capturedRenderer.dispose();
          if (capturedCanvas && container.contains(capturedCanvas)) {
            container.removeChild(capturedCanvas);
          }
        }
        if (capturedMaterial) capturedMaterial.dispose();
        if (capturedGeometry) capturedGeometry.dispose();
        _activeCleanupTimer = null;
      }, 300);
    };
  }, [webGLSupported, quality, interactive, pillarRotation, bottomColor, topColor, glowAmount, intensity, noiseIntensity, pillarHeight, pillarWidth]);
